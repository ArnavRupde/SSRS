...
#     try:
#         cur = conn.cursor()
#         cur.execute('''INSERT INTO futuresales2 (productId,stock,updatedOn) VALUES (?, ?,?)''', (pid, predictionss,today))
#         conn.commit()
...
#         msg="error occured"
#         cur = conn.cursor()
#         cur.execute("""UPDATE futuresales2 SET stock = ?  WHERE productId= ? """, (predictionss,pid))
#         print("Update Done")
#         conn.commit()
#         conn.rollback()
# conn.close()
...
print(sys.argv[1])
pid=int(sys.argv[1])
csv_filename=str(pid)+".csv"
print(csv_filename)
apple =  pd.read_csv(csv_filename, header=0)
...
plt.legend()
filename_to_saveas=str("static/uploads/")+str(pid)+".jpg"
print(filename_to_saveas)
plt.savefig(filename_to_saveas, bbox_inches='tight')
plt.show()
...
with sqlite3.connect('database.db') as conn:
    try:
        ...
        msg="error occured"
        cur = conn.cursor()
        cur.execute("""UPDATE futuresales2 SET stock = ?,updatedOn = ? WHERE productId= ? """, (predictionss,today,pid))
        print("Update Done")
        conn.commit()
        conn.rollback()
conn.close()
